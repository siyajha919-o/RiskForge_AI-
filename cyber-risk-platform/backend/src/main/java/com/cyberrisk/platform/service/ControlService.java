...
public class ControlService {
    ...
    public ControlResponse create(ControlRequest request) {
        Asset asset = assetRepository.findById(request.assetId())
                .orElseThrow(() -> new EntityNotFoundException("Asset not found: " + request.assetId()));
        ...
        Control control = Control.builder()
                ...
                .frameworkMappings(request.frameworkMappings() == null ? new ArrayList<>() : request.frameworkMappings())
                .build();
        return ControlResponse.from(controlRepository.save(control));
    }

    @Transactional(readOnly = true)
    public List<ControlResponse> findByAsset(UUID assetId) {
        return controlRepository.findByAssetId(assetId).stream()
                .map(ControlResponse::from).collect(Collectors.toList());
    }
