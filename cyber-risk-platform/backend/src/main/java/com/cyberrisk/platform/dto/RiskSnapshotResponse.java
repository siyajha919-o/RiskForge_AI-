...
public record RiskSnapshotResponse(
    ...
    public static RiskSnapshotResponse from(RiskSnapshot s) {
        return new RiskSnapshotResponse(s.getAsset().getId(), s.getAnnualizedRateOfOccurrence(),
                s.getSingleLossExpectancy(), s.getExpectedAnnualLoss(), s.getValueAtRisk95(),
                s.getOpenFindingsCount(), s.getCalculatedAt());
    }
}
